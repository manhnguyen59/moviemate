    ...
    <div class="app-card border app-border rounded-2xl overflow-hidden shadow-lg">
        ...
        <div class="p-5 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="app-card rounded-3xl p-4 border app-border">
                    <div class="text-xs uppercase tracking-wider app-muted mb-2">Tổng đồ ăn</div>
                    <div class="text-2xl font-bold">{{ number_format($order->total_amount, 0, ',', '.') }}đ</div>
                    <div class="text-xs app-muted">Trạng thái: {{ ucfirst($order->status) }}</div>
                    @if($order->booking)
                        <div class="text-xs app-muted mt-2">Booking: <a class="text-brand-start font-bold" href="{{ route('admin.bookings.show', $order->booking) }}">{{ $order->booking->booking_code }}</a></div>
                    @endif
                </div>
            </div>
                    ...
                    @foreach($order->items as $item)
                        <div class="py-3 grid grid-cols-12 gap-4 items-center">
                            <div class="col-span-6">
                                ...
                                <div class="text-sm app-muted">{{ $item->quantity }} × {{ number_format($item->price, 0, ',', '.') }}đ</div>
                            </div>
                            <div class="col-span-6 text-right font-semibold">{{ number_format($item->total, 0, ',', '.') }}đ</div>
                        </div>
                    @endforeach
                </div>
            </div>
